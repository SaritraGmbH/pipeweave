...
export async function requestMaintenance(db: Database): Promise<MaintenanceStatus> {
  ...
  console.log('[maintenance] Requested maintenance mode');
  console.log(`[maintenance] Waiting for ${counts.pending + counts.running} tasks to complete`);
...
export async function enterMaintenance(db: Database): Promise<MaintenanceStatus> {
  ...
  console.log('[maintenance] Entered maintenance mode');
...
export async function exitMaintenance(db: Database): Promise<OrchestratorState> {
  ...
  if (state.mode === 'running') {
    console.log('[maintenance] Already in running mode');
    return state;
  }

  const newState = await setOrchestratorMode(db, 'running');
  console.log('[maintenance] Exited maintenance mode - orchestrator is now running');
